...
func (m *Service) FromComposeConfig(
	...
	if len(service.Volumes) > 0 {
		...
		for _, v := range service.Volumes {
			volume := ServiceVolume{
				...
				Bind:     types.ObjectNull(VolumeBind{}.AttrType()),
			}
			if v.ReadOnly {
	...
	if len(service.DependsOn) > 0 {
		...
		for k, v := range service.DependsOn {
			...
			if v.Condition != "" {
				dependency.Condition = types.StringValue(v.Condition)
			}
			if v.Restart {
				dependency.Restart = types.BoolValue(true)
			}
	...
	if service.Networks != nil {
		...
		for k, v := range service.Networks {
			...
			// Only hydrate aliases the user explicitly declared. Docker adds the
			// service name as an implicit network alias at runtime, but injecting
			// it here would make the Aliases attribute (Optional, not Computed)
			// non-null when the plan left it unset, tripping "inconsistent result
			// after apply" on the sensitive services attribute.
			if len(v.Aliases) > 0 {
