...
func normalizeComposeProject(project *composetypes.Project) {
	...
	// Networks intentionally do NOT get their name defaulted to the map key.
	// The name attribute is Optional (not Computed), so a config that leaves it
	// unset plans as null; injecting the key here would make the post-apply
	// value non-null and trip "inconsistent result after apply". An explicit
	// name set by the user already round-trips through the content YAML.
